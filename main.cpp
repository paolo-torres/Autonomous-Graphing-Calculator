...
int parseFunction(string function, double constants[], int cExp = MAXDEG) {
	...
	/* if the function is a polynomial, recursively call the parseInt function
	on polynomials one degree less, storing the coefficient each time */	
	if(current == "x") {
		if(cExp != 1) {
		//	cout << function.substr(strIndex+2,function.length()) << endl;
		...
		type = 1;
		//cout << function.substr(strIndex+3,function.length()) << endl;
		double tempPoly[5] = {0};
...
void findYValues(double yVals[RES], double xMin, double xMax, int type, double con[5])
{
	...
	{
		if(type==0)
			yVals[y] = con[0]*pow(x,4) + con[1]*pow(x,3) + con[2]*pow(x,2) + 
			con[3]*pow(x,1) + con[4];
		else if(type==1) //Assume sine format: F(x)=A*Sin(Bx+C)+D *same with cos
			yVals[y] = con[0]*sin(con[1]*x+con[2])+con[3];
			//sin((con[1]*x-con[2])/180*M_PI) for degrees
		else if(type==2) 
